...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("manage_alerts:"):
        await log_button_trace(context, cid, data, "MANAGE_ALERTS_START")
        token_ref = data.split(":", 1)[1]
        token_key = resolve_token_ref_robust(cid, token_ref, context)
        await log_button_trace(context, cid, data, "MANAGE_ALERTS_RESOLVED", str(token_key))
        entry = db.get_tracked_token(cid, token_key)
        if not entry:
            tracked_now = db.get_tracked(cid)
            if len(tracked_now) == 1:
        ...
        await log_button_trace(context, cid, data, "MANAGE_ALERTS_DELIVERED", str(delivered))
    ...
    if data.startswith("pref|"):
        ...
        token_key = resolve_token_ref_robust(cid, token_ref, context)
        # FIX: log when ref resolution fails so it's visible in production logs
        if token_key == token_ref:
            log.warning(
                f"pref: ref={token_ref!r} could not be resolved for cid={cid}. "
                f"callback_token_map={db.get_user(cid).get('callback_token_map')}"
            )
